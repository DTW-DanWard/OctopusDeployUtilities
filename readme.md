...
* **It post-processes your export data to simplify and improve usage.**  For example it automatically adds id -> name lookup information so you can view/work with a variable scope with it's user-friendly name, i.e. ```EnvironmentName = 'Production-West'``` instead of only by Id, i.e. ```Environment = 'Environments-37'```.  It also adds deploy process and variable configuration directly to each project file.  And a lot more!
