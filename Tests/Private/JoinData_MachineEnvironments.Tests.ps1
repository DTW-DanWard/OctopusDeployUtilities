...
Describe 'Update environment add machines' {
  ...
  It 'no parameter throws error' {
    { Update-ODUExportAddMachinesToEnvironment } | Should throw
  }

  It 'null parameter throws error' {
    { $BadPath = $null; Update-ODUExportAddMachinesToEnvironment -Path $BadPath } | Should throw
  }

  It 'bad path throws error' {
    { Update-ODUExportAddMachinesToEnvironment -Path (Join-Path -Path $TestExportRootPath -ChildPath FolderNotFound) } | Should throw
  }

  It 'before update environment does not have machine property' {
    $Export = Read-ExportFromFile -Path $TestExportPath
    $Export.Environments[0] | Get-Member -Name MachineIds -MemberType NoteProperty | Should BeNullOrEmpty
  }

  It 'after update environment has machine property' {
    Update-ODUExportAddMachinesToEnvironment -Path $TestExportPath
    $Export = Read-ExportFromFile -Path $TestExportPath
    $Export.Environments[0] | Get-Member -Name MachineIds -MemberType NoteProperty | Should Not BeNullOrEmpty
  }
}
#endregion
