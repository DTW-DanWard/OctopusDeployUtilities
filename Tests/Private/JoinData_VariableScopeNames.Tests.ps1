...
#region Update variables add scope names
Describe 'Update variables add scope names' {
  ...
  It 'no parameter throws error' {
    { Update-ODUExportAddScopeNamesToVariable } | Should throw
  }

  It 'null parameter throws error' {
    { $BadPath = $null; Update-ODUExportAddScopeNamesToVariable -Path $BadPath } | Should throw
  }

  It 'bad path throws error' {
    { Update-ODUExportAddScopeNamesToVariable -Path (Join-Path -Path $TestExportRootPath -ChildPath FolderNotFound) } | Should throw
  }

  It 'before update variable does not have scope Breadth' {
    $Export = Read-ExportFromFile -Path $TestExportPath
    # the variable identified by that Id has custom scope settings
    ($Export.Variables.Variables | Where-Object { $_.Id -eq '7f6f8c5c-f34e-ef55-7fe3-9514d014f946' }).Scope | Get-Member -Name Breadth -MemberType NoteProperty | Should BeNullOrEmpty
  }

  It 'after update variable has scope Breadth' {
    Update-ODUExportAddScopeNamesToVariable -Path $TestExportPath
    $Export = Read-ExportFromFile -Path $TestExportPath
    ($Export.Variables.Variables | Where-Object { $_.Id -eq '7f6f8c5c-f34e-ef55-7fe3-9514d014f946' }).Scope | Get-Member -Name Breadth -MemberType NoteProperty | Should Not BeNullOrEmpty
  }
}
#endregion
